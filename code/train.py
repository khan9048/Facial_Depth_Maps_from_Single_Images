...
from utils.criterion import SiLogLoss, scale_invariant_loss, make_mask
...
def validate(val_loader, model, criterion_d, device, epoch, args, log_dir):
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        # masking valied area
        valid_mask, final_mask = make_mask(preds.squeeze(), depth_gt, args.dataset)

        valid_out = preds[valid_mask]
        valid_gt_sparse = depth_gt[valid_mask]

        ###################################### scale invariant loss #####################################
        scale_inv_loss = scale_invariant_loss(valid_out, valid_gt_sparse)
        #################################################################################################

        depth_loss.update(scale_inv_loss.item(), input_RGB.size(0))
